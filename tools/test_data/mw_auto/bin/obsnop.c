...
void scan_input(char *buffer, input_t *elem) {
    ...
    printf("kp: (%d %d %d %d %d, %d %d %d %d %d), s: %d, c: %d, d: %d\n",
            ...
            elem->KP_4, elem->KP_5, elem->KP_6, elem->KP_7, elem->KP_8, 
            elem->KP_9, elem->KP_CLEAR, elem->KP_START);
#endif
}
...
FILE* create_trace_file(char* fname) {
    ...
    fprintf(fp, "enable,LEFT_DIGIT,SETUP,COOKING,SUSPENDED,START_PRESSED,"
...
int main(int argc, char** argv){
    /* testing facility */
#ifndef TRACE_FNAME
#define TRACE_FNAME "output_trace.csv"
#endif
    if (argc != 2) {
	...
	long n_times_to_run = NO_OF_TIMES_TO_RUN;
    int i;
	for(i=0; i<n_times_to_run; i++) {
        FILE* fp_dump = create_trace_file(TRACE_FNAME);
